...
dd_filter = FILTER date_dim BY (d_date >= '20010621') AND (d_date <= '20010721');

dd_proy = FOREACH dd_filter GENERATE d_date, d_date_sk;

--it_group = GROUP it_proy BY i_item_sk;

query59_DdCa = JOIN dd_proy BY d_date_sk, catalog_sales BY cs_sold_date_sk;
