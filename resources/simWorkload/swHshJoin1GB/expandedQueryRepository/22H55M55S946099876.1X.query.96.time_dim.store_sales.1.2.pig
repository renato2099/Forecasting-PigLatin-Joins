...
td_filter = FILTER td_proy BY (t_hour == 8) AND (t_minute>=30);

query96_TdSs = JOIN td_filter BY t_time_sk, ss_proy BY ss_sold_time_sk;

STORE query96_TdSs INTO 'queryResults/query.96.time_dim.store_sales.1.2' USING PigStorage('|');

rm queryResults/query.96.time_dim.store_sales.1.2;
