...
ss_proy = FOREACH store_sales GENERATE ss_item_sk;

query52_ItSs = JOIN ss_proy BY ss_item_sk, it_proy BY i_item_sk USING 'replicated';

STORE query52_ItSs INTO 'queryResults/query.52.store_sales.item.3.1' using PigStorage('|');

rm queryResults/query.52.store_sales.item.3.1;
