...
ss_proy = FOREACH store_sales GENERATE  ss_store_sk;
sd_proy = FOREACH store_dim GENERATE s_store_sk;

query96_SdSs = JOIN ss_proy BY ss_store_sk, sd_proy BY s_store_sk USING 'replicated';

STORE query96_SdSs INTO 'queryResults/query.96.store_sales.store.8.1' USING PigStorage('|');

rm queryResults/query.96.store_sales.store.8.1;
