...
--pr_filter = FILTER promotion BY (p_channel_email == 'N') OR (p_channel_event == 'N');
pr_proy = FOREACH promotion GENERATE p_promo_sk;
ss_proy = FOREACH store_sales GENERATE ss_promo_sk;

query7_PrSs = JOIN pr_proy BY p_promo_sk, ss_proy BY ss_promo_sk;

STORE query7_PrSs INTO 'queryResults/query7_PrSs' USING PigStorage('|');

rm queryResults/query7_PrSs;
