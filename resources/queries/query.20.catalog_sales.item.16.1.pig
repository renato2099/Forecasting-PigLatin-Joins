...
it_filter = FILTER item BY (i_category == 'Shoes') OR (i_category == 'Electronics') OR (i_category == 'Home');

it_proy = FOREACH it_filter GENERATE i_item_desc, i_category, i_class, i_current_price, i_item_sk;

--it_group = GROUP it_proy BY i_item_sk;

query59_ItCa = JOIN it_proy BY i_item_sk, catalog_sales BY cs_item_sk;

STORE query59_ItCa INTO 'queryResults/query.20.catalog_sales.item.16.1' using PigStorage('|');

rm queryResults/query.20.catalog_sales.item.16.1;
